...
export default class CLI extends events.EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this.options.success = this.options.success || (logsymbols.success + ' Build success in %sms');

    this.options.name = this.options.name || process.argv[1].split('/').slice(-1)[0];

    let alias = this.alias;
    this.argv = minimist(this.options.argv || process.argv.slice(2), { alias });
  ...
  help(targets = {}) {
    ...
    if (Object.keys(targets).length) targetList += leftpad.slice(0, -2) + 'Targets:\n';
  ...
  pad(str) {
    return new Array(PADDING - str.length).join(' ');
  }
  ...
  ${options.options}
    ...
    if (options.commands) console.log(options.commands);
